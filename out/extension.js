...
var MAX_READFILE_SIZE = 2 * 1024 * 1024 * 1024;
var API_REQUEST_TIMEOUT_MS = 15e3;
var API_MAX_REDIRECTS = 5;
var API_MAX_NETWORK_RETRIES = 3;
var API_RETRY_BASE_DELAY_MS = 1e3;
...
async function getAccessTokenViaPython(dbPath) {
  ...
  const pythonScript = `import sqlite3, sys; conn = sqlite3.connect(sys.argv[1]); cur = conn.cursor(); cur.execute("SELECT value FROM ItemTable WHERE key = 'cursorAuth/accessToken' LIMIT 1"); row = cur.fetchone(); print(row[0] if row and row[0] else ''); conn.close()`;
...
function execFileAsync(command, args) {
  return new Promise((resolve, reject) => {
    (0, import_child_process.execFile)(command, args, { windowsHide: true, maxBuffer: 1024 * 1024 }, (error, stdout) => {
      if (error) {
        reject(error);
        return;
      }
...
function isFileTooLargeError(error) {
  ...
  return error.code === "ERR_FS_FILE_TOO_LARGE";
}
function sleep(ms) {
...
function isRetryableNetworkError(error) {
  ...
  const message = getErrorMessage(error);
  return /Client network socket disconnected before secure TLS connection was established/i.test(message) || /socket hang up/i.test(message);
}
...
async function fetchUsageFromAPI(userId, retryOnAuth = true) {
